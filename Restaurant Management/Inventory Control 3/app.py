...
(function () {
    ...
    // Validate folder path before sending to backend
    function validateRenameFolderPath() {
        var folderPath = document.getElementById('renameFolderPath').value;
        var warning = document.getElementById('renameCsvWarning');
        if (!folderPath || folderPath.length < 3 || !folderPath.match(/^[A-Za-z]:\\/)) {
            if (warning) {
                warning.style.display = '';
                warning.textContent = 'Please select a valid folder path.';
            }
            return false;
        }
    ...
    // Patch preview/apply functions to validate first
    var origPreviewOrderCsvRename = window.previewOrderCsvRename;
    window.previewOrderCsvRename = function () {
        if (!validateRenameFolderPath()) return;
        if (origPreviewOrderCsvRename) origPreviewOrderCsvRename();
    };
    var origApplyOrderCsvRename = window.applyOrderCsvRename;
    window.applyOrderCsvRename = function () {
        if (!validateRenameFolderPath()) return;
        if (origApplyOrderCsvRename) origApplyOrderCsvRename();
    };
