...
class ReportApp(tk.Tk):
    ...
    def _build_filters(self):
        ...
        # --- Cash Deductions Report Button ---
        cash_deductions_btn = tk.Button(filter_frame, text="Show Cash Deductions",
                                        command=self._show_cash_deductions,
                                        bg="#2c7a7b", fg="white",
                                        activebackground="#14505c", activeforeground="white",
                                        font=("Helvetica", 12, "bold"),
                                        relief="flat", cursor="hand2", height=2)
        cash_deductions_btn.pack(pady=(0, 15), padx=15, fill="x")
        cash_deductions_btn.bind("<Enter>", lambda e: cash_deductions_btn.config(bg="#14505c"))
        cash_deductions_btn.bind("<Leave>", lambda e: cash_deductions_btn.config(bg="#2c7a7b"))
    ...
    def _show_cash_deductions(self):
        """Fetch and display total cash deductions for selected date range"""
        import requests
        for widget in self.results_container.winfo_children():
            widget.destroy()
        from_date = self.from_date.get_date().strftime("%Y-%m-%d")
        to_date = self.to_date.get_date().strftime("%Y-%m-%d")
        try:
            # Replace with actual API URL if running locally or deployed
            api_url = f"http://127.0.0.1:5000/api/reports/cash-deductions?start_date={from_date}&end_date={to_date}"
            response = requests.get(api_url)
            if response.status_code == 200:
                result = response.json()
                if result.get('success'):
                    total = result.get('total_deductions', 0.0)
                    daily = result.get('daily', [])
                    # Display summary
                    summary_frame = tk.Frame(self.results_container, bg="#e8f4f8", relief="solid", bd=1)
                    summary_frame.pack(fill="x", padx=10, pady=10)
                    tk.Label(summary_frame, text="Cash Deductions Summary",
                            font=("Helvetica", 12, "bold"),
                            bg="#e8f4f8", fg="black").pack(pady=(8, 5))
                    tk.Label(summary_frame, text=f"Total Cash Deductions: ${total:,.2f}",
                            font=("Helvetica", 11), bg="#e8f4f8", fg="black").pack(pady=(0, 8), padx=10)
                    # Daily breakdown
                    if daily:
                        daily_frame = tk.Frame(self.results_container, bg="#f9f9f9", relief="solid", bd=1)
                        daily_frame.pack(fill="x", padx=10, pady=10)
                        tk.Label(daily_frame, text="Daily Breakdown",
                                font=("Helvetica", 11, "bold"), bg="#f9f9f9", fg="black").pack(pady=(8, 5))
                        for entry in daily:
                            tk.Label(daily_frame, text=f"{entry['date']}: ${entry['deductions']:,.2f}",
                                    font=("Helvetica", 10), bg="#f9f9f9", fg="black").pack(anchor="w", padx=10)
                else:
                    tk.Label(self.results_container, text="No data found for selected range.",
                            font=("Helvetica", 11), bg="#f9f9f9", fg="black").pack(pady=30)
            else:
                tk.Label(self.results_container, text=f"API error: {response.status_code}",
                        font=("Helvetica", 11), bg="#f9f9f9", fg="black").pack(pady=30)
        except Exception as e:
