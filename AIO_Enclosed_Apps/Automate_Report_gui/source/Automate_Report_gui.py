...
def _week_sort_key(sheet_name):
    upper = sheet_name.upper().replace(" ", "")
    quarter_match = re.search(r"Q([1-4])W(\d+)", upper)
    if quarter_match:
        return (0, int(quarter_match.group(1)), int(quarter_match.group(2)), sheet_name)

    week_match = re.search(r"W(?:EEK)?(\d+)", upper)
    if week_match:
        return (1, 0, int(week_match.group(1)), sheet_name)

    return (2, 0, 9999, sheet_name)


def _detect_year_from_text(text, fallback_year):
    if not text:
...
def _sheet_looks_like_week_report(sheet):
    name_upper = sheet.title.upper().replace(" ", "")
    if re.search(r"Q[1-4]W\d+", name_upper) or re.search(r"W(?:EEK)?\d+", name_upper):
        return True

    markers = set()
    max_row = min(180, sheet.max_row)
    max_col = min(60, sheet.max_column)
    for row in sheet.iter_rows(min_row=1, max_row=max_row, min_col=1, max_col=max_col):
        for cell in row:
            value = cell.value
            if isinstance(value, str):
                label = value.strip().lower()
                if label in {"total week forecast", "total week actual", "sales by shift"}:
                    markers.add(label)
        if len(markers) >= 2:
            return True
    return False
...
class QuarterlyReportGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Quarterly Report Generator")
        self.root.geometry("750x650")
        
        # Variables
        self.source_file = tk.StringVar()
        self.output_file = tk.StringVar()
        self.quarter_name = tk.StringVar(value="Q1 2026")
        self.locations_var = tk.StringVar(value="Kingsville, Alice")
        self.week_sheets_var = tk.StringVar(value="Q1W1, Q1W2, Q1W3, Q1W4, Q1W5")
        
        self.create_widgets()
        
    def create_widgets(self):
        # Title
        title_label = tk.Label(self.root, text="Quarterly Report Generator", 
                              font=("Arial", 16, "bold"), bg="#1F4E78", fg="white", pady=10)
        ...
        # Source File
        ttk.Label(main_frame, text="Source Excel File:", font=("Arial", 10, "bold")).grid(row=0, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.source_file, width=40).grid(row=1, column=0, columnspan=2, pady=5)
        ttk.Button(main_frame, text="Browse", command=self.browse_source).grid(row=1, column=2, padx=5)
        ttk.Button(main_frame, text="Auto-Detect", command=self.auto_detect_settings, style="Accent.TButton").grid(row=1, column=3, padx=5)
        
        # Output File
        ttk.Label(main_frame, text="Output Excel File:", font=("Arial", 10, "bold")).grid(row=2, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.output_file, width=40).grid(row=3, column=0, columnspan=2, pady=5)
        ttk.Button(main_frame, text="Browse", command=self.browse_output).grid(row=3, column=2, padx=5)
        
        # Quarter Name
        ttk.Label(main_frame, text="Quarter Name (e.g., Q1 2026):", font=("Arial", 10, "bold")).grid(row=4, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.quarter_name, width=40).grid(row=5, column=0, columnspan=4, pady=5)
        
        # Locations (Fixed)
        ttk.Label(main_frame, text="Locations (Fixed - Kingsville, Alice):", font=("Arial", 10, "bold")).grid(row=6, column=0, sticky=tk.W, pady=5)
        locations_entry = ttk.Entry(main_frame, textvariable=self.locations_var, width=40, state="readonly")
        locations_entry.grid(row=7, column=0, columnspan=4, pady=5)
        
        # Week Sheets
        ttk.Label(main_frame, text="Week Sheet Names (comma-separated):", font=("Arial", 10, "bold")).grid(row=8, column=0, sticky=tk.W, pady=5)
        week_text_frame = ttk.Frame(main_frame)
        week_text_frame.grid(row=9, column=0, columnspan=4, pady=5, sticky=tk.W+tk.E)
        
        self.week_sheets_text = tk.Text(week_text_frame, width=60, height=6, wrap=tk.WORD)
        self.week_sheets_text.insert("1.0", self.week_sheets_var.get())
        self.week_sheets_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        scrollbar = ttk.Scrollbar(week_text_frame, command=self.week_sheets_text.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.week_sheets_text.config(yscrollcommand=scrollbar.set)
        
        # Info Label
        info_label = ttk.Label(main_frame, text="💡 Tip: Use 'Auto-Detect' to scan Quarter Name and Week Sheets. Locations are always Kingsville, Alice.", 
                               foreground="blue", font=("Arial", 9, "italic"))
        info_label.grid(row=10, column=0, columnspan=4, pady=5)
        
        # Progress
        self.progress_label = ttk.Label(main_frame, text="", foreground="green")
        self.progress_label.grid(row=11, column=0, columnspan=4, pady=10)
    ...
    def browse_source(self):
        ...
        if filename:
            ...
            if not self.output_file.get():
                ...
                self.output_file.set(f"{base} - Updated{ext}")
            
            # Automatically detect settings when file is selected
            if messagebox.askyesno("Auto-Detect", "Would you like to automatically detect Quarter Name and Week Sheets from this file?"):
    ...
    def auto_detect_settings(self):
        """Automatically detect Quarter Name and Week Sheets from the source file"""
        ...
        try:
            self.progress_label.config(text="Detecting settings... Please wait...", foreground="blue")
            self.root.update()
            
            # Load the workbook
            wb = openpyxl.load_workbook(self.source_file.get(), read_only=True, data_only=True)
            
            # Detect week sheet names by both sheet name and table markers
            week_sheets = []
            for sheet in wb.worksheets:
                if _sheet_looks_like_week_report(sheet):
                    week_sheets.append(sheet.title)

            # Remove duplicates and sort naturally (Q1W1, Q1W2, ..., Q1W10)
            week_sheets = sorted(list(dict.fromkeys(week_sheets)), key=_week_sort_key)

            # Infer quarter + year from sheet names and source file name
            detected_year = _detect_year_from_text(os.path.basename(self.source_file.get()), datetime.now().year)
            quarter_name = f"Q1 {detected_year}"  # default fallback

            detected_quarter = None
            for sheet_name in week_sheets:
                quarter_match = re.search(r"Q\s*([1-4])", sheet_name.upper())
                if quarter_match:
                    detected_quarter = quarter_match.group(1)
                    break

            if detected_quarter:
                quarter_name = f"Q{detected_quarter} {detected_year}"
            elif week_sheets:
                quarter_name = f"{detected_year} Weekly Report"
            else:
                quarter_name = f"Q1 {detected_year}"
            
            wb.close()
            
            # Update the GUI fields
            if week_sheets:
                self.week_sheets_text.delete("1.0", tk.END)
                self.week_sheets_text.insert("1.0", ", ".join(week_sheets))
                detected_info = f"✓ Detected {len(week_sheets)} week sheets"
            else:
                detected_info = "⚠ No week sheets detected"
            
            self.quarter_name.set(quarter_name)
            detected_info += f"\n✓ Quarter name: {quarter_name}"
            detected_info += "\n✓ Locations: Kingsville, Alice (fixed)"
            
            self.progress_label.config(text=detected_info, foreground="green")
            messagebox.showinfo("Auto-Detection Complete", detected_info)
            
        except Exception as e:
            self.progress_label.config(text="✗ Detection failed", foreground="red")
            messagebox.showerror("Error", f"Failed to auto-detect settings:\n{str(e)}")
    ...
    def generate_report(self):
        ...
        # Get values
        locations = [loc.strip() for loc in self.locations_var.get().split(",") if loc.strip()]
        week_sheets_text = self.week_sheets_text.get("1.0", tk.END)
        week_sheets = [sheet.strip() for sheet in week_sheets_text.replace("\n", ",").split(",") if sheet.strip()]
        ...
        if not week_sheets:
            messagebox.showerror("Error", "Please enter at least one week sheet name")
            return
        
        self.progress_label.config(text="Generating report... Please wait...")
        self.root.update()
...
def _find_location_blocks(data_ws, locations):
    forecast_rows = []
...
def read_weekly_locations(data_ws, sheet_name, locations):
    """Extract totals for each location within a weekly sheet."""
    results = []
    for location, forecast_row, end_row in _find_location_blocks(data_ws, locations):
        if not location:
            continue
...
def create_overview_sheet(source_file, output_file, quarter_name, locations, week_sheets):
    ...
    for sheet_name in week_sheets:
        ...
        for row in read_weekly_locations(data_ws, sheet_name, locations):
            row["week"] = sheet_name
    ...
    ws["A1"] = f"{quarter_name} QUARTERLY REPORT OVERVIEW"
    ...
    for location in locations:
        ...
        current_row += 1
        for data in rows:
            ws.cell(row=current_row, column=1, value=data["week"]).alignment = Alignment(horizontal="center", vertical="center")
        ...
        current_row += 1
        # All Morning shifts
        day_totals = {day: 0 for day in day_headers[1:-1]}
        row_count = 0
        for data in rows:
            ws.cell(row=current_row, column=1, value=data["week"]).alignment = Alignment(horizontal="center", vertical="center")
            week_total = 0
            for idx, day in enumerate(day_headers[1:-1], start=2):
                ...
                ws.cell(row=current_row, column=idx).alignment = Alignment(horizontal="center", vertical="center")
                week_total += value
            ...
            ws.cell(row=current_row, column=len(day_headers)).alignment = Alignment(horizontal="center", vertical="center")
            current_row += 1
            row_count += 1
        ...
        # Night Shift by Day Section - similar structure
        ...
        for shift_label, key in [("Morning", "morning_by_day"), ("Night", "night_by_day")]:
            ...
            ws.cell(row=current_row, column=1, value=shift_label).alignment = Alignment(horizontal="center", vertical="center")
            ws.cell(row=current_row, column=2, value=best_day).alignment = Alignment(horizontal="center", vertical="center")
            ws.cell(row=current_row, column=3, value=averages[best_day]).number_format = "$#,##0.00"
            ws.cell(row=current_row, column=3).alignment = Alignment(horizontal="center", vertical="center")
            ws.cell(row=current_row, column=4, value=worst_day).alignment = Alignment(horizontal="center", vertical="center")
