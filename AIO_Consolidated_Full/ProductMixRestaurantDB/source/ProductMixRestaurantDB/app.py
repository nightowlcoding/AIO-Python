...
def _get_production_items(conn, restaurant_id):
    ...
    rows = conn.execute(
        """
        SELECT p.id, p.item_name, p.master_item_id, p.count_mode, p.case_pack_units, p.weight_oz_per_unit, m.item_name AS master_item_name
        FROM production_items p
        LEFT JOIN master_items m ON m.id = p.master_item_id
...
def _upsert_production_item(conn, restaurant_id, item_name, master_item_id=None, count_mode="unit", case_pack_units=None, weight_oz_per_unit=None):
    cleaned_name = _clean_text(item_name, 180)
    if not cleaned_name:
        return None
    ...
    conn.execute(
        """
        INSERT INTO production_items (restaurant_id, item_name, master_item_id, count_mode, case_pack_units, weight_oz_per_unit)
        VALUES (?, ?, ?, ?, ?, ?)
...
def update_production_item_case_pack(item_id):
    ...
    restaurant_ids = _get_sync_group_restaurant_ids(scoped_restaurant) or [int(scoped_restaurant["id"])]
    placeholders = ",".join("?" for _ in restaurant_ids)
    conn.execute(
