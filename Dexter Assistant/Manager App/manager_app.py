...
def _load_persistent_secret_key() -> str:
	"""Return a stable SECRET_KEY across restarts.

	Resolution order:
	  1. MGR_SECRET_KEY environment variable.
	  2. .mgr_secret file beside this module.
	  3. Newly generated token persisted to .mgr_secret on first run.
	"""
	env_key = os.environ.get('MGR_SECRET_KEY')
	if env_key:
		return env_key
	secret_path = os.path.join(os.path.dirname(__file__), '.mgr_secret')
	try:
	...
	try:
		...
		# Fall back to in-memory key if disk write fails; sessions will not
		# survive restart but the app still runs.
		pass
	return new_key
...
# Manager App session cookie from overwriting Dexter's session cookie.
app.config['SESSION_COOKIE_NAME'] = 'mgr_session'
app.config['SESSION_COOKIE_PATH'] = '/app/managerapp/'
app.config['REMEMBER_COOKIE_NAME'] = 'mgr_remember_token'
app.config['REMEMBER_COOKIE_PATH'] = '/app/managerapp/'

# Flask-Limiter setup for rate limiting
def _rate_limit_key() -> str:
    """Rate-limit key aware of Dexter proxy headers.

    When requests are proxied from Dexter, remote_addr is typically 127.0.0.1,
    which would collapse all users into one shared limiter bucket. Prefer the
    forwarded Dexter user identity when present.
    """
...
def is_strong_password(password):
	"""Check for strong password: 8+ chars, upper, lower, number, symbol."""
	return (
		len(password) >= 8 and
...
def _load_shared_restaurants_from_productmix() -> list[dict]:
    ...
    for row in rows:
        ...
        if not label:
            continue
        options.append({'id': f"pm_{int(row['id'])}", 'name': label})
    return options
...
def _find_active_user_for_dexter_bridge(username, email):
    """Return an active user row matching Dexter username/email, or None."""
...
def _is_trusted_dexter_bridge_request():
    """Accept bridge headers only for local proxy traffic."""
    if request.headers.get('X-Dexter-Auth') != '1':
        return False
...
def _provision_user_for_dexter_bridge(username, email, is_admin):
    ...
    for idx in range(0, 20):
        ...
        user_id, _ = db.create_user(
            ...
            candidate_email,
            generated_password,
            full_name=candidate_username,
            is_system_admin=bool(is_admin),
        )
        if user_id:
            conn = db.get_connection()
            try:
                cursor = conn.cursor()
                cursor.execute(
                    'UPDATE users SET accepted_terms_version = ?, accepted_terms_date = ? WHERE id = ?',
                    ('dexter-sso', now, user_id),
                )
                conn.commit()
            finally:
                conn.close()
            return _find_active_user_for_dexter_bridge(candidate_username, candidate_email)

    return None

# ==================== DATA STORAGE HELPERS ====================
def create_employee_json(company_id, employee, location_id=None):
	"""Create a JSON file for a new employee with basic info, discipline, and certifications."""
	if location_id:
		emp_dir = f"company_data/{company_id}/locations/{location_id}/employees"
	else:
...
@app.before_request
def bridge_dexter_session_into_manager_app():
    """Single sign-on bridge: if Dexter already authenticated the user,
    auto-login the matching Manager App account without showing a second login."""
    ...
    user_data = _find_active_user_for_dexter_bridge(dexter_user, dexter_email)
    if not user_data:
        user_data = _provision_user_for_dexter_bridge(dexter_user, dexter_email, dexter_is_admin)
        if not user_data:
            return None
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not is_strong_password(password):
            flash('Password must be at least 8 characters and include upper, lower, number, and symbol.', 'warning')
            return render_template('register.html')
...
def daily_log():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Get location from form or session
            location_id = request.form.get('location_id') or selected_location
    ...
    try:
        ...
        for emp in employees:
            ...
            if emp.get('firstName') and emp.get('lastName') and emp.get('dateOfHire'):
                ...
                create_employee_json(company_id, emp, location_id)
